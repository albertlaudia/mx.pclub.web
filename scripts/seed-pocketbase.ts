...
async function pbUpdate<T>(col: string, id: string, data: Record<string, unknown>): Promise<T> {
  ...
  if (!res.ok) throw new Error(`update ${col}/${id}: ${res.status} ${(await res.text()).slice(0, 300)}`)
  return (await res.json()) as T
}
...
const APPS: Array<{
...
}> = [
  {
    ...
    features: [
      { title: "Daily drop", description: "A new book every morning, ready when you are.", icon: "Sparkles" },
      { title: "Audio + text", description: "Read or listen — whichever fits the moment.", icon: "Headphones" },
      { title: "Insights that stick", description: "Key ideas revisit you throughout the week.", icon: "Lightbulb" },
  ...
  },
  {
    ...
    features: [
      { title: "3-minute resets", description: "For the moment you need it, not 20-minute rituals.", icon: "Clock" },
      { title: "Clinically informed", description: "Designed with real psychologists, not wellness clichés.", icon: "Stethoscope" },
      { title: "Warm human voice", description: "Calm guidance that sounds like a person, not a robot.", icon: "Mic" },
      { title: "Truly private", description: "Your sessions stay on your device. No tracking, ever.", icon: "Shield" },
    ],
    platforms: ["ios", "android"],
    download_ios: "",
    download_android: "",
  ...
  },
  {
    ...
    features: [
      { title: "One tiny win", description: "A 5-minute prompt that actually moves the needle.", icon: "Target" },
      { title: "Evidence-based", description: "Built on habit research, not hustle influencers.", icon: "BookOpen" },
      { title: "No streak shame", description: "Miss a day, the app doesn't even notice.", icon: "Heart" },
  ...
  },
  {
    ...
    features: [
      { title: "Studio accuracy", description: "±0.5 cent precision. The pros use it.", icon: "Crosshair" },
      { title: "Beautiful visualizer", description: "Tuning feedback that actually feels good.", icon: "Waves" },
      { title: "Zero tracking", description: "No analytics, no crash reports, no network calls.", icon: "ShieldCheck" },
      { title: "Forever free", description: "No ads, no in-app purchases, no \"pro\" tier.", icon: "Gift" },
    ],
    platforms: ["ios", "android"],
    download_ios: "",
...
const POSTS: StaticPost[] = [
  {
    ...
    excerpt: "Money is a story we tell ourselves. These five books rewrite the story — and most are under 250 pages.",
  ...
  },
  {
    ...
    excerpt: "If your guitar sounds off no matter what you do, it's almost certainly this. The fix takes 30 seconds.",
  ...
  },
  {
    ...
    excerpt: "The 5am club is a brand, not a blueprint. Here's the morning routine that fits your actual life.",
  ...
  },
  {
    ...
    excerpt: "Privacy isn't a feature you bolt on. It's a design constraint that produces better apps.",
...
const TOPICS: Array<{ slug: string; name: string; description: string; color: string; icon: string; sort_order: number }> = [
  { slug: "books", name: "Books & learning", description: "What to read, and what to skip.", color: "#F5A623", icon: "BookOpen", sort_order: 1 },
  { slug: "wellness", name: "Wellness & calm", description: "For the harder moments.", color: "#7FB069", icon: "Heart", sort_order: 2 },
  { slug: "habits", name: "Habits & routine", description: "Small moves, lasting change.", color: "#E07856", icon: "Sunrise", sort_order: 3 },
  { slug: "music", name: "Music & craft", description: "For the makers.", color: "#C46A4A", icon: "Music", sort_order: 4 },
  { slug: "meta", name: "Field notes", description: "How we build, why we build.", color: "#6B6B6B", icon: "Notebook", sort_order: 5 },
]

const AUTHORS: Array<{ slug: string; name: string; bio: string; role: string; twitter?: string; github?: string }> = [
  {
    slug: "albert-l",
    name: "Albert L.",
...
async function main() {
  ...
  for (const post of POSTS) {
    ...
    // Look up author ID
    const authors = await pbList<{ id: string; slug: string }>("pc_authors", `slug = '${post.author}'`)
    const authorId = authors[0]?.id ?? ""

    const r = await upsertByFilter<{ id: string }>(
      "pc_blog_posts",
      `slug = '${post.slug}'`,
      {
        slug: post.slug,
    ...
    console.log(`  ${r.action === "created" ? "+" : "~"} ${post.slug} (${r.id}, ${body.length}b body)`)
  }
